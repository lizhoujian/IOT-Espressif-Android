...
public class EspDevice implements IEspDevice, Cloneable {
	...
	/**
	 * empty device1 is used by EspDeviceCacheHandler to distinguish between
	 * Internet unaccessible and user with no devices
	 */
	public static EspDevice EmptyDevice1 = new EspDevice();

	/**
	 * empty device2 is used by EspDeviceCacheHandler to distinguish between
	 * Internet unaccessible and user's device list is empty
	 */
	...
	public void saveInDB() {
		IOTDeviceDBManager iotDeviceDBManager = IOTDeviceDBManager
				.getInstance();
		int type = this.getDeviceType().getSerial();
		int state = this.getDeviceState().getStateValue();
		iotDeviceDBManager.insertOrReplace(mDeviceId, mDeviceKey, mBssid, type,
				state, mIsOwner, mDeviceName, mRomVersion, mLatestRomVersion,
				mTimeStamp, mActivatedTime, mUserId, mProductId);
	}
	...
	public boolean isStateEqual(IEspDevice device) {
		...
		return this.getDeviceState().getStateValue() == device.getDeviceState()
				.getStateValue();
	}
	...
	public void copyDeviceState(IEspDevice device) {
		this.mDeviceState
				.setStateValue(device.getDeviceState().getStateValue());
	}

	@Override
	public void copyDeviceRomVersion(IEspDevice deivce) {
		IEspDeviceUpgradeParser parser = EspDeviceUpgradeParser.getInstance();
		IEspDeviceUpgradeInfo deviceUpgradeInfoCurrent = parser
				.parseUpgradeInfo(this.mRomVersion);
		IEspDeviceUpgradeInfo deviceUpgradeInfoNew = parser
				.parseUpgradeInfo(deivce.getRom_version());
		...
		if (deviceUpgradeInfoCurrent == null
				|| deviceUpgradeInfoNew.getVersionValue() > deviceUpgradeInfoCurrent
						.getVersionValue()) {
	...
	public String toString() {
		return "EspDevice: (mBssid=[" + mBssid + "],mParentDeviceBssid=["
				+ mParentDeviceBssid + "]mDeviceId=[" + mDeviceId
				+ "],mDeviceName=[" + mDeviceName + "],mDeviceState=["
				+ mDeviceState + "],mIsMeshDevice=[" + mIsMeshDevice
				+ "],mInetAddress=[" + mInetAddress + "]" + "],mProductId=["
				+ mProductId + "])";
	}

	private List<IEspDeviceTreeElement> __getDeviceTreeElementListByBssid2(
			List<IEspDevice> allDeviceList) {
		if (!allDeviceList.contains(this)) {
			throw new IllegalStateException(
					"allDeviceList don't contain current device");
		}
		...
		do {
			...
			do {
				...
				if (parentIndex != -1) {
					IEspDevice parentDevice = allMeshDeviceList
							.get(parentIndex);
					...
					for (int childIndex = 0; childIndex < allDeviceCount; ++childIndex) {
						IEspDevice childDevice = allMeshDeviceList
								.get(childIndex);
						String childParentBssid = childDevice
								.getParentDeviceBssid();
		...
		for (int i = 0; i < allDeviceCount; ++i) {
			...
			if (level != -1) {
				...
				IEspDeviceTreeElement deviceTreeElement = new EspDeviceTreeElement(
						device, parentDeviceKey, hasParent, hasChild, level);
