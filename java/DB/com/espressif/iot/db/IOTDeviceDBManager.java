...
public class IOTDeviceDBManager implements IDeviceDBManager,
		IEspSingletonObject {
	private final static Logger log = Logger
			.getLogger(IOTDeviceDBManager.class);
	...
	private long getNextNegativeId() {
		Query<DeviceDB> query = deviceDao.queryBuilder()
				.where(Properties.Id.lt(0)).build();
		long nextId = 0;
		...
		--nextId;
		log.debug(Thread.currentThread().toString() + "##getNextNegativeId(): "
				+ nextId);
		return nextId;
	}
	...
	private long getNegativeDeviceIdByBssid(String bssid) {
		Query<DeviceDB> query = deviceDao.queryBuilder()
				.where(Properties.Bssid.eq(bssid)).build();
		DeviceDB device = query.unique();
		if (device == null) {
			log.info(Thread.currentThread().toString()
					+ "##getNegativeDeviceIdByBssid(bssid=[" + bssid + "]): 0");
			return 0;
		} else {
			long result = device.getId();
			log.info(Thread.currentThread().toString()
					+ "##getNegativeDeviceIdByBssid(bssid=[" + bssid + "]): "
					+ result);
			return result;
		}
	}
	...
	private DeviceDB getDeviceById(long deviceId) {
		Query<DeviceDB> query = deviceDao.queryBuilder()
				.where(Properties.Id.eq(deviceId)).build();
		DeviceDB result = query.unique();
		log.debug(Thread.currentThread().toString()
				+ "##getDeviceById(deviceId=[" + deviceId + "]): " + result);
		return result;
	}
	...
	public synchronized void renameDevice(long deviceId, String name) {
		...
		if (device != null) {
			...
			log.info(Thread.currentThread().toString()
					+ "##renameDevice(deviceId=[" + deviceId + "],name=["
					+ name + "])");
		} else {
			log.debug(Thread.currentThread().toString()
					+ "##renameDevice(deviceId=[" + deviceId + "],name=["
	...
	@Override
	public synchronized long insertActivatingDevice(String key, String bssid,
			int type, int state, String name, String rom_version,
		...
		DeviceDB deviceDB = new DeviceDB(deviceId, key, bssid, type, state,
				true, name, rom_version, latest_rom_version, timestamp, 0,
				userId, productId);
		log.info(Thread.currentThread().toString()
				+ "##insertActivactingDevice(deviceId=[" + deviceId
				+ "],bssid=[" + bssid + "],type=[" + type + "],state=[" + state
				+ "],isOwner=[" + true + "],name=[" + name + "],rom_version=["
				+ rom_version + "],latest_rom_version=[" + latest_rom_version
	...
	@Override
	public synchronized void insertOrReplace(long deviceId, String key,
			String bssid, int type, int state, boolean isOwner, String name,
			String rom_version, String latest_rom_version, long timestamp,
			long activatedTime, long userId, long productId) {
		DeviceDB deviceDB = new DeviceDB(deviceId, key, bssid, type, state,
				isOwner, name, rom_version, latest_rom_version, timestamp,
				activatedTime, userId, productId);
		deviceDao.insertOrReplace(deviceDB);
		log.info(Thread.currentThread().toString()
				+ "##insertOrReplace(deviceId=[" + deviceId + "],bssid=["
				+ bssid + "],type=[" + type + "],state=[" + state
				+ "],isOwner=[" + isOwner + "],name=[" + name + "],timestamp=["
				+ timestamp + "],userId=[" + userId + "],productId=["
				+ productId + "])");
	}
	...
	public synchronized void delete(long deviceId) {
		...
		if (result != null) {
			...
			log.info(Thread.currentThread().toString() + "##delete(deviceId=["
					+ deviceId + "]): " + result);
		} else {
			log.debug(Thread.currentThread().toString() + "##delete(deviceId=["
					+ deviceId + "]): the device isn't exist");
		}
	}
	...
	public void deleteDevicesByDeviceId(long deviceId) {
		...
		if (result != null) {
			...
			log.debug(Thread.currentThread().toString()
					+ "##deleteDevicesByDeviceId(deviceId=[" + deviceId
	...
	public void deleteDevicesByBssid(String bssid) {
		...
		if (!deviceList.isEmpty()) {
			...
			log.info(Thread.currentThread().toString()
					+ "##deleteDevicesByDeviceId(bssid=" + bssid + "]):"
					+ deviceList);
		}
	}
}
