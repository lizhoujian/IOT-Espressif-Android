...
public class EspActionDeviceUpgradeOnline implements
		IEspActionDeviceUpgradeOnline {
	private final static Logger log = Logger
			.getLogger(EspActionDeviceUpgradeOnline.class);

	private static boolean __isHttpsSupported() {
		SharedPreferences sp = EspApplication.sharedInstance()
				.getSharedPreferences(EspStrings.Key.SYSTEM_CONFIG,
	...
	private IEspDevice __getCurrentDevice(String deviceKey) {
		...
		if (HttpStatus.SC_OK == status) {
			...
			try {
				...
				is_owner_key = true;
				// deviceJson
				JSONObject deviceJson = jsonObjectResult
						.getJSONObject("device");
				long id = deviceJson.getLong("id");
				long productId = deviceJson.getLong("product_id");
				...
				if (deviceName.length() > "device-name-".length()) {
					String deviceNamePre = deviceName.substring(0,
							"device-name-".length());
				...
				String latest_rom_version = deviceJson
						.getString("latest_rom_version");
				EspDeviceState state = new EspDeviceState();
				state.addStateInternet();
				IEspDevice device = BEspDevice.getInstance().alloc(deviceName,
						id, token, is_owner_key, BSSID, state.getStateValue(),
						ptype, rom_version, latest_rom_version, userId,
						productId);
				boolean isParentMdevMacValid = !deviceJson
						.isNull(Parent_Mdev_Mac);
				String parentBssid = null;
				if (isParentMdevMacValid) {
					// parent device bssid
					String parentDeviceBssid = deviceJson
							.getString(Parent_Mdev_Mac);
					if (!parentDeviceBssid.equals("null")) {
						parentBssid = MeshUtil
								.getRawMacAddress(parentDeviceBssid);
					}
				}
				...
				log.debug(Thread.currentThread().toString()
						+ "##__getCurrentDevice(deviceKey=[" + deviceKey
						+ "]): " + device);
				return device;
			} catch (JSONException e) {
				e.printStackTrace();
	...
	/**
	 * get the current device, and check whether its
	 * romVersion==LatestRomVersion which indicate the upgrade is finished
	 * 
	 * @param deviceKey
	 *            the device key
	...
	private IEspDevice __checkDeviceUpgradeOnlineSuc(String deviceKey) {
		...
		if (device != null) {
			...
			if (romVersion != null && latestRomVersion != null
					&& romVersion.equals(latestRomVersion)) {
				return device;
			} else {
				return null;
			}
	...
	private IEspDevice checkDeviceUpgradeOnlineSuc(String deviceKey) {
		...
		while (device == null) {
			try {
				log.debug(Thread.currentThread().toString()
						+ "##checkDeviceUpgradeOnlineSuc(deviceKey=["
						+ deviceKey + "]): sleep " + RETRY_TIME_MILLISECONDS
						+ " milliseconds");
	...
	private boolean deviceUpgradeOnline(String deviceKey,
		...
		if (HttpStatus.SC_OK == status) {
			log.debug(Thread.currentThread().toString()
					+ "##deviceUpgradeOnline(deviceKey=[" + deviceKey
					+ "],latestRomVersion=[" + latestRomVersion + "]): "
					+ "true");
			return true;
		} else {
			log.debug(Thread.currentThread().toString()
	...
	private void rebootDeviceOnline(String deviceKey) {
		...
		if (HttpStatus.SC_OK == status) {
			log.info(Thread.currentThread().toString()
					+ "##deviceRebootOnline(deviceKey=[" + deviceKey + "]) ok");
			// return true;
		} else {
			log.warn(Thread.currentThread().toString()
					+ "##deviceRebootOnline(deviceKey=[" + deviceKey
					+ "]) fail");
		}
		// return false;
	}
