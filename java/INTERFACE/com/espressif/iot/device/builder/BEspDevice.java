...
public class BEspDevice implements IBEspDevice {
	...
	@Override
	public IEspDevice alloc(String deviceName, long deviceId, String deviceKey,
			boolean isOwner, String bssid, int state, int ptype,
			String rom_version, String latest_rom_version, long userId,
			long productId, long... timestamp) {
		...
		switch (deviceType) {
		case NEW:
			// if we get device from local, we don't care the Activating
			// device's ssid and wifiCipher type
	...
	public IEspDevice alloc(IDeviceDB deviceDB) {
		...
		long productId = deviceDB.getProductId();
		long timestamp = deviceDB.getTimestamp();
		long activatedTime = deviceDB.getActivatedTime();
		IEspDevice device = alloc(deviceName, deviceId, deviceKey, isOwner,
				bssid, state, ptype, rom_version, latest_rom_version, userId,
				productId, timestamp);
