...
public class EspCommandDeviceNewActivateInternet implements
		IEspCommandDeviceNewActivateInternet {
	private final static Logger log = Logger
			.getLogger(EspCommandDeviceNewActivateInternet.class);
	...
	private boolean isDeviceOnline(EspDeviceType deviceType, long last_active,
			long currentTime) {
		long timeout = 0;
		switch (deviceType) {
		case FLAMMABLE:
		...
		/**
		 * when last_active is after currentTime or currentTime - last_active <=
		 * timeout, the device is online
		 */
	...
	@Override
	public IEspDevice doCommandNewActivateInternet(long userId, String userKey,
			String randomToken) {

		JSONObject jsonObject = new JSONObject();
		try {
			jsonObject.put(Token, randomToken);
			HeaderPair header = new HeaderPair(Authorization, Token + " "
					+ userKey);
			...
			if (status == HttpStatus.SC_OK) {
				...
				// usually when the device could be activated suc, it won't get
				// Long.MIN_VALUE,
				...
				long productId = Long.parseLong(key.getString("product_id"));
				// deviceJson
				...
				long last_active = deviceJson.getLong("last_active")
						* TimeUtil.ONE_SECOND_LONG_VALUE;
				boolean isOnline = isDeviceOnline(
						EspDeviceType.getEspTypeEnumBySerial(ptype),
				...
				if (name.length() > "device-name-".length()) {
					String deviceNamePre = name.substring(0,
							"device-name-".length());
					if (deviceNamePre.equals("device-name-")) {
						name = BSSIDUtil.genDeviceNameByBSSID(BSSID);
					}
				}
				IEspDevice device = BEspDevice.getInstance().alloc(name, id,
						token, is_owner_key, BSSID, state.getStateValue(),
				...
				log.debug(Thread.currentThread().toString()
						+ "##doCommandNewActivateInternet(userId=[" + userId
						+ "],userKey=[" + userKey + "],randomToken=["
						+ randomToken + "]): " + device);
				return device;
			}
		...
		log.warn(Thread.currentThread().toString()
				+ "##doCommandNewActivateInternet(userId=[" + userId
				+ "],userKey=[" + userKey + "],randomToken=[" + randomToken
				+ "]): " + null);
		return null;
	}
